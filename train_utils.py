                ...
                with torch.no_grad():
                    with tqdm(range(0, len(val_data), batch_size), leave=False, desc="Validating") as pbar:
                        ...
                        for b in pbar:
                            ...
                            # detach pred
                            if isinstance(val_preds[-1], list) or isinstance(val_preds[-1], tuple):
                                val_preds[-1] = list(val_preds[-1])
                            ...
                            # detach target
                            if isinstance(val_y[-1], list) or isinstance(val_y[-1], tuple):
                                val_y[-1] = list(val_y[-1])
